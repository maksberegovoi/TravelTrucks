...
const Modal = ({ children, isOpen, toggle }) => {
  ...
  return (
    <div
      className={`${!isOpen && "hidden"} absolute inset-0 w-full h-screen bg-gray-500 opacity-70`}
    >
      <button onClick={toggle} className="interactive absolute top-5 right-5">
        <svg className="w-[40px] h-[40px] text-black" id="icon-close" viewBox="0 0 20 20">
